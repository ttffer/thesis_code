...
proc createNplace_bondpads {args} {
  ...
  if {[info exists pargs(-inline_pad_ref_name)]} {
   ...
   foreach io_cell $all_io_cell_list {
      ...
      set io_cell_X_offset 66.00
      set io_cell_Y_offset 66.00
      set io_cell_stagger_LR_X_offset 104.00
      set io_cell_stagger_TB_Y_offset 104.00
      set io_cell_stagger_LR_Y_offset 10.95
      set io_cell_stagger_TB_X_offset 10.95
      ...
      if { $io_cell_orient == "E" } {
         ##set io_cell_orient_flip FE
	 if {$stagger == "true" && ![expr $left_i % 2]} {

	     create_cell $bond_pad_name $stagger_bond_pad_ref_name
             set bond_pad_LL_X [expr $io_cell_LL_X - $io_cell_X_offset - $io_cell_stagger_LR_X_offset]
             set bond_pad_LL_Y [expr $io_cell_LL_Y - $io_cell_stagger_LR_Y_offset]
         
         } else {

	    create_cell $bond_pad_name $bond_pad_ref_name
	    set bond_pad_LL_X [expr $io_cell_LL_X - $io_cell_X_offset]
            set bond_pad_LL_Y [expr $io_cell_LL_Y - $io_cell_stagger_LR_Y_offset]
	 ...
	 set left_i [expr $left_i + 1]
      ...
      ## top side io cell
      if { $io_cell_orient == "S" } {
	 ##set io_cell_orient_flip FN
	 if {$stagger == "true" && ![expr $top_i % 2]} {

	     create_cell $bond_pad_name $stagger_bond_pad_ref_name
	     set bond_pad_LL_X [expr $io_cell_LL_X - $io_cell_stagger_TB_X_offset]
             set bond_pad_LL_Y [expr $io_cell_UR_Y - $stagger_pad_height + $io_cell_Y_offset + $io_cell_stagger_TB_Y_offset]

	 } else {
	 
	     create_cell $bond_pad_name $bond_pad_ref_name
             set bond_pad_LL_X [expr $io_cell_LL_X - $io_cell_stagger_TB_X_offset]
             set bond_pad_LL_Y [expr $io_cell_UR_Y  - $pad_height + $io_cell_Y_offset]
         }
      ...
      ## right side io cell
      if { $io_cell_orient == "W" } {
	 ##set io_cell_orient_flip FW
	 if {$stagger == "true" && ![expr $right_i % 2]} {

	     create_cell $bond_pad_name $stagger_bond_pad_ref_name
	     set bond_pad_LL_X [expr $io_cell_UR_X - $stagger_pad_height + $io_cell_X_offset + $io_cell_stagger_LR_X_offset]
             set bond_pad_LL_Y [expr $io_cell_LL_Y - $io_cell_stagger_LR_Y_offset] 
	    ...
	    create_cell $bond_pad_name $bond_pad_ref_name
	    set bond_pad_LL_X [expr $io_cell_UR_X - $pad_height + $io_cell_X_offset]
            set bond_pad_LL_Y [expr $io_cell_LL_Y - $io_cell_stagger_LR_Y_offset]
	...
	set right_i [expr $right_i + 1]
      ...
      if { $io_cell_orient == "N" } {
         ##set io_cell_orient_flip FS
	 if {$stagger == "true" && ![expr $bottom_i % 2]} {

	     create_cell $bond_pad_name $stagger_bond_pad_ref_name
             set bond_pad_LL_X [expr $io_cell_LL_X - $io_cell_stagger_TB_X_offset]
             set bond_pad_LL_Y [expr $io_cell_LL_Y - $io_cell_Y_offset - $io_cell_stagger_TB_Y_offset]

	 } else {
	
	     create_cell $bond_pad_name $bond_pad_ref_name
	     set bond_pad_LL_X [expr $io_cell_LL_X - $io_cell_stagger_TB_X_offset]
             set bond_pad_LL_Y [expr $io_cell_LL_Y - $io_cell_Y_offset]
    ...
    ##set_attribute -quiet $bond_pad_name orientation $io_cell_orient_flip
   }
